...
render() { # render <site> <script> <query-param> <value>
  (cd "$1" && $PHP -r '$_SERVER += ["SCRIPT_NAME"=>$argv[1],"HTTP_HOST"=>"localhost","REQUEST_URI"=>"/","SERVER_PORT"=>"80"];
                       $_GET = [$argv[2] => $argv[3]]; include basename($argv[1]);' -- "$2" "$3" "$4" 2>/dev/null)
}
for site in ping4 ping6 dumpers; do
  ...
  grep -q 'rel="canonical"' <<<"$landing" || { echo "$site: no canonical"; exit 1; }
